      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
              <div class="p-6 text-gray-900">
                ...
                <div class="py-12">
                  <form action="{{route('gadget.update', $post->id)}}" method='POST' class="flex flex-col gap-3 justify-center">
                      @csrf
                      @method('PUT')
              
                      
                      <div class="form-group row">
                          <label for="gadgetType" class="block text-gray-700 text-sm font-bold mb-2" for="gadgetType">Gadget Type</label>
                          <div class="col-sm-10">
                            <input type="gadgetType" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="brand" type="text" name="gadgetType" placeholder="Gadget Type..." value="{{$post->gadgetType}}">
                            @error('gadgetType')
                              <div>{{$message}}</div>
                            @enderror
                          </div>
